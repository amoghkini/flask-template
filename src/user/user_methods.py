...
class UserMethods:
    ...
    def add_new_user(user: User) -> None:
        try:
            ...
            conn.insert(DatabaseSchema.FLASK_TEMPLATE, DatabaseTables.USERS, user_dict)
            conn.commit()
    ...
    def change_password(user: Dict,
        ...
        UserMethods.update_user_data(user.get('username'), fields_to_update)
    ...
    def get_user(email: str) -> Dict:
        try:
            conn = get_db()
            user = conn.get_one(DatabaseSchema.FLASK_TEMPLATE, DatabaseTables.USERS, '*', ("email_id = %s", [email]))
            return user
        except Exception as e:
    ...
    def get_user_by_username(username: str) -> Dict:
        try:
            ...
            user = conn.get_one(DatabaseSchema.FLASK_TEMPLATE, DatabaseTables.USERS, '*', ("username = %s", [username]))
            return user
        except Exception as e:
    ...
    def update_user_data(user_identifier: str,
        ...
        conn = get_db()
        user = conn.update(DatabaseSchema.FLASK_TEMPLATE, DatabaseTables.USERS, fields_to_update, (by+"=%s", (user_identifier,))) 
        if user:
            conn.commit()
            return user
        else:
    ...
    def validate_change_password(user: Dict,
        ...
        if reset:
            ...
            existing_user = UserMethods.get_user_by_username(user.get('username'))
    ...
    def get_profile_data(form):
        ...
        user_db_data: Dict = UserMethods.get_user_by_username(username)
        if not user_db_data:
    ...
    def convert_profile_form_to_dict(form) -> Dict:
        try:
            ...
            user_form_data['username'] = form.username.data
